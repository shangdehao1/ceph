...
void CacheSession::read_request_data(uint64_t mid_len) {
...
void CacheSession::handle_request_data(bufferptr bp, uint64_t mid_len,  
...
void CacheSession::send(ObjectCacheRequest* reply) {
    ...
    reply->m_mid_buffer.clear();
    reply->encode();
    bufferlist bl;
    bl.append(reply->get_head_buffer());
    bl.append(reply->get_mid_buffer());
    ...
    boost::asio::async_write(m_dm_socket,
        ...
        [this, bl, reply](const boost::system::error_code& err, size_t cb) {
          if(err || cb != bl.length()) {
            fault();
            return;
          }
