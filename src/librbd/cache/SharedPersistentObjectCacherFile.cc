...
int SyncFile::open(int flag, std::string temp_name) {
  ...
  if(m_fd == -1) {
    std::cout << "SyncFile::open fails : " << strerror(errno) << std::endl;
    return -errno;
  }
  return m_fd;
}
...
int SyncFile::rename(std::string old_name, std::string new_name) {
  ...
  if(ret != 0) {
    std::cout <<" SyncFile::rename fails : "<< strerror(errno) << std::endl;
  } 
...
{}
 ...
 void WriteSyncFileRequest::finish(int r) {
  ...
  if(ret < 0) {
    if(m_op_work_queue != nullptr && m_on_finish != nullptr) {
      m_on_finish->complete(-1);
    } else {
      m_ret = ret;
    }
...
void DeleteSyncFileRequest::finish(int r) {
  ...
  ret = m_sync_file.remove(temp_name);
  if(ret < 0) {
    if(m_op_work_queue != nullptr && m_on_finish != nullptr) {
      m_on_finish->complete(-1);
    } else {
